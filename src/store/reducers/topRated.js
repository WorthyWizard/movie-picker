...
const topRatedReducer = (state = initialState, action) => {
  switch(action.type) {
    case actionTypes.SET_TOP_RATED_MOVIES: return setMovies(state, action);
    case actionTypes.TOP_RATED_MOVIES_FETCH_START: return fetchStart(state, action);
    case actionTypes.TOP_RATED_MOVIES_FETCH_SUCCESS: return fetchSuccess(state, action);
    case actionTypes.TOP_RATED_MOVIES_FETCH_FAIL: return fetchFail(state, action);
